...
function Post({ arrayName, onToggleReaction, onTogglePostMark, onEdit, post, showUserData, editButtons }) {
  ...
  const addReaction = async () => {
    if (!auth.currentUser) {
      dispatch(showNotification({
        id: nanoid(), type: 'Error', text: 'Войдите чтобы оставить реакцию'
      }));
    ...
    try {
      ...
      dispatch(showNotification({
        id: nanoid(), type: 'Error', text: 'Ошибка добавления реакции'
      }));
    }
  }
  ...
  const markPost = async () => {
    if (!auth.currentUser) {
      dispatch(showNotification({
        id: nanoid(), type: 'Error', text: 'Войдите чтобы добавить в избранное'
      }));
    ...
    try {
      ...
      dispatch(showNotification({
        id: nanoid(), type: 'Error', text: 'Ошибка добавления в избранное'
      }));
    }
  }
      ...
      <div ref={postContentRef} className="relative overflow-hidden" style={(showFullContent === 1) ? { 'height': '500px' } : {}}>
        ...
        {showFullContent === 1 &&
          <div className="absolute z-10 top-[420px] flex justify-center w-full bg-gradient-to-t from-[white] pt-12 pb-1">
            <p className="text-lg text-neutral-4 cursor-pointer" onClick={() => { setShowFullContent(2) }}>Больше ...</p>
          </div>}
        {showFullContent === 2 &&
          <div className="flex justify-center w-full p-1 bg-[white]">
            <p className="text-lg text-neutral-4 cursor-pointer" onClick={() => { setShowFullContent(1) }}>Скрыть</p>
          </div>}
      </div>
