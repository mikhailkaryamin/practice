...
function useSortPosts(userFilteringTopics, arrayName) {
  ...
  if (arrayName !== 'allPosts') {
    ...
    if (sortCriteria === 'Реакции' && sortOrder === 1) {
      sortedPosts.sort((a, b) => a.reactions.length - b.reactions.length);
    }
    else if (sortCriteria === 'Реакции' && sortOrder === 2) {
      sortedPosts.sort((a, b) => b.reactions.length - a.reactions.length);
    }
    else if (sortCriteria === 'Дата публикации' && sortOrder === 1) {
      sortedPosts.sort((a, b) => b.publishDate - a.publishDate);
    }
    else if (sortCriteria === 'Дата публикации' && sortOrder === 2) {
    ...
    if (filterByPopularity === 'Популярные') {
      sortedPosts.sort((a, b) => b.reactions.length - a.reactions.length);
    }
    else if (filterByPopularity === 'Избранные') {
