...
function UserPosts({ arrayName }) {
  ...
  if (userPosts[arrayName].length === 0) {
    ...
    switch (arrayName) {
      case 'ownPosts':
        noPostsText = 'У тебя нет постов';
        break;
      case 'subscriptions':
        noPostsText = 'У тебя нет подписчиков';
        break;
      case 'marked':
        noPostsText = 'У тебя нет избранных постов';
        break;
      default:
        noPostsText = 'У пользователя нет постов';
        break;
    }
    ...
    <section className="flex flex-col items-center grow bg-[white] rounded-xl shadow-lg">
      <header className={headerClass}>
        ...
        {!addEditPostMode &&
          <div className="flex space-x-4 xl:order-3 xl:self-center xl:justify-center xl:w-full md:order-3 md:self-center md:justify-center md:w-full">
            <SortCriteria title="Реакции" onSort />
            <SortCriteria title="Дата публикации" onSort />
          </div>}

        {(addEditPostMode) ?
          <div className="xl:order-2 md:order-2"><MultipleSelect value={blogTopics} onChange={(value) => { setBlogTopics(value) }}
            title="Темы" options={TOPICS_LIST} /></div> :
      ...
      {!addEditPostMode &&
        <footer className="flex justify-around items-center w-full p-4">
          <p className="text-neutral-4">Количество постов: {userPosts[arrayName].length}</p>
          <p className="text-neutral-4">Количество реакций: {reactions}</p>
        </footer>}
    </section>
  );
}
