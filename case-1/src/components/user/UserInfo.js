...
function UserInfo() {
  ...
  const subscribe = async () => {
    ...
    try {
      ...
      dispatch(showNotification({
        id: nanoid(), type: 'Error', text: 'Ошибка подписки'
      }));
    }
  }
  ...
  return (
    <div className="flex flex-col items-center self-start space-y-2 p-6 bg-[white] rounded-xl shadow-lg lg:self-center">
      ...
      <p>{userData.subscribers.length} подписан</p>
      <p>{userData.subscriptions.length} подписчики</p>

      <div className="flex items-center space-x-2">
        <ReactIcon src={<BiSolidCalendar className="w-8 h-8" />} />
        <p className="whitespace-nowrap">Присоединен {formatDate(userData.creationTime)}</p>
      </div>
      ...
      <div className="w-full pt-2">
        <Button className="w-full bg-primary hover:bg-primarySaturated" onClick={subscribe}>
          ...
          <span>{(userData.subscribers.indexOf(auth.currentUser?.uid) === -1) ? 'Подписаться' : 'Отписаться'}</span>
        </Button>
      </div>
    </div>
  );
